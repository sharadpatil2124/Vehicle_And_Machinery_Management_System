...
async function list(req, res) {
  const response = await machineryService.listMachinery({ tenantId: req.auth.tenantId, query: req.query });
  res.status(200).json({ ...response, message: 'Machinery retrieved' });
}

async function get(req, res) {
  const data = await machineryService.getMachine({ tenantId: req.auth.tenantId, id: req.params.id });
...
async function history(req, res) {
  const data = await machineryService.getMachineHistory({ tenantId: req.auth.tenantId, id: req.params.id });
